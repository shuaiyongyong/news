...
<%!
	...
	public List< Map<String, Object>> getAllNews() throws Exception {
...
<body>
		...
		<div id="opt_area">
			...
			<form action="news/update.action" method="post" id="addNews" enctype="multipart/form-data">
				<%
					...
					List<Map<String,Object>> newsTopic=getAllNews();
				...
				<p>
					<label> 标题 </label> <input name="ntitle" type="text" value="<%= news.get("NTITLE") %>" class="opt_input" />
				</p>
				<p>
					<label> 作者 </label> <input name="nauthor" type="text" value="<%= news.get("NAUTHOR") %>" class="opt_input" />
				</p>
				<p>
					<label> 摘要 </label>
				...
				<p>
					<label> 上传图片 </label> 
					<input name="file" type="file" onchange="addPic(this)" multiple="multiple"/><br>
					<img id="image" alt="图片" src="<%=news.get("NPICPATH")==null?"images/94337_205.jpg":news.get("NPICPATH") %>" width="100">
				</p>
				<p id="pics"></p>
				
				<input name="nid" type="hidden" value="<%=news.get("NID") %>"> <!-- 隐藏 的标签,但是数据会被传递 -->
	...
	<script type="text/javascript">
		//2.异步取到所有主题信息
		/* $.post("control/topicControll.jsp", function(data) {//data是个对象
			
			var currValue=$("#ntid").val();
			$("#ntid").empty();
			for (var i = 0; i < data.length; i++) {
				$("#ntid").append("<option value='" + data[i].TID  + "'"+(currValue == data[i].TID?"selected":"")+">" + data[i].TNAME + "</option>");
			}
		}, "json");
		
		function addPic(obj){
			var picURI = window.URL.createObjectURL(obj.files[0]);//根据图片资源创建一个图片路径
			$("#image").attr("src", picURI);
		} */
	</script>
</body>
</html>
